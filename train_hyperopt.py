...
my_space = {
    'experiment.module.model.encoder_name': hp.choice('experiment.module.model.encoder_name', ['resnet50', 'resnet101']),
}
...
def objective_with_args(hparams, params, config_path):
    ...
    params['experiment']['module']['model']['encoder_name'] = hparams['experiment']['module']['model']['encoder_name']
    # cfg['training']['target_size'] = hparams['target_size']
...
#    params['training']['learning_rate'] = hparams['learning_rate']
    ...
    return {'loss': -mlrun.data.metrics["test_JaccardIndex"], 'status': STATUS_OK}
...
def main(cfg, config_path):
    ...
    # ToDo: Customize where the trials file is
    root_path = Path(cfg.experiment.datamodule.root_dir)
    ...
    objective = partial(objective_with_args, params=cfg, config_path=config_path)

    n = 0
    while n < cfg.hyperopt.runs:
        best = fmin(objective,
                    space=my_space,
                    algo=tpe.suggest,
                    trials=trials,
                    max_evals=n + cfg['global']['hyperopt_delta'])
        n += cfg.hyperopt.delta
